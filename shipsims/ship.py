...
class Ship():
    def __init__(self, x, y, width, height, goal_thresh = 10, max_time = 60, do_draw = False):
    ...
    def update(self, dt : float):
        ...
        self.velocity[0] += self.MAXTHRUST * dt * self.thrust
        self.velocity[1] += self.MAXTHRUST * dt * self.hthrust
    ...
    def update_parts(self, dt):
        if self.do_draw:
            thrust = (self.thrust**2 + self.hthrust**2)**.5
            self.partsys.create_particle(
                p.Particle(self.x, self.y,
                -self.thrust*800 + self.velocity[0], -self.hthrust*800 + self.velocity[1], 10*thrust,
    ...
    def get_inputs(self):
        ...
        return [self.x - self.goal_positions[self.goal_counter%l][0],
                self.y - self.goal_positions[self.goal_counter%l][1], self.velocity[0], self.velocity[1]]

    def set_controls(self, controls = []):
        self.thrust = max(-1, min((controls[0]*2) - 1, 1))
        self.hthrust = max(-1, min((controls[1]*2) - 1, 1))
