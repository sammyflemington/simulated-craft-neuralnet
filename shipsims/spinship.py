...
class SpinShip():
    def __init__(self, x, y, width, height, goal_thresh = 20, max_time = 60, do_draw = False):
    ...
    def get_inputs(self):
        ...
        return [self.x - self.goal_positions[self.goal_counter%l][0],
                self.y - self.goal_positions[self.goal_counter%l][1], self.velocity[0], self.velocity[1], self.angle]#, self.angular_velocity]

    def set_controls(self, controls = []):
        self.thrust = max(0, min((controls[0]*2) - 1, 1))
        self.angular_thrust = max(-1, min((controls[1]*2) - 1, 1))
